(function() {
  ...
  DesktopUploader = (function(_super) {
    ...
    /* Private properties */
    var cache, checkIgnore, configPath, customConfig, extensions, loadConfig, log, name, paths, queue, saveConfig, self, throttleGroup, update, upload, watcher, _saveConfig;
    ...
    checkIgnore = function(filename) {
      var stat, _ref;
      if (extensions && (_ref = filename.split('.').pop().toLowerCase(), __indexOf.call(extensions, _ref) < 0)) {
        return true;
      }
      if (!cache[filename]) {
        return false;
